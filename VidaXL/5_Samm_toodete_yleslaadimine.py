...
class WooCommerceUploader:
    def __init__(self):
        ...
        self.category_translation_path = base / "category_translation.json"
        self.category_translation = self._load_category_translation()
    ...
    def _load_category_translation(self) -> Dict[str, str]:
        if not self.category_translation_path.exists():
            return {}
        try:
            data = json.load(self.category_translation_path.open('r', encoding='utf-8'))
    ...
    def _fetch_category_cache(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                for item in items:
                    ...
                    self.category_cache.setdefault(key, cid)
                if len(items) < per_page:
                    break
                page += 1
        except Exception as e:
            print(f"   ⚠️  Category cache fetch failed: {e}")

    def _get_category_id(self, name: str, parent_id: int) -> Optional[int]:
        key = f"{parent_id}:{name.lower()}"
        return self.category_cache.get(key)

    def _create_category(self, name: str, parent_id: int) -> Optional[int]:
        ...
        if resp.status_code == 400:
            try:
                ...
                if existing_id:
                    key = f"{parent_id}:{name.lower()}"
                    self.category_cache[key] = existing_id
                    print(f"   ℹ️  Category already exists: {name} (ID {existing_id})")
                    return existing_id
            except Exception:
                pass
        print(f"   ❌ Failed to create category '{name}': HTTP {resp.status_code} {resp.text[:200]}")
        return None

    def ensure_category_path(self, full_path: str) -> Optional[int]:
        parts = self._split_path(full_path)
        if not parts:
            return None
        self._fetch_category_cache()
        parent_id = 0
        for segment in parts:
            if not segment:
                continue
            existing = self._get_category_id(segment, parent_id)
            if existing:
                parent_id = existing
                continue
            created = self._create_category(segment, parent_id)
            if not created:
                return None
            parent_id = created
        return parent_id
    ...
    def create_product_payload(self, product: dict, product_exists: bool = False, status: str = "publish", update_images: bool = False, sku_override: str = None):
        ...
        # Price mapping rule: always enforce minimum (purchase_price * (1+markup) * (1+VAT))
        # and round up to .99
        meta_lookup = {}
        for m in in_meta:
        ...
        # Base prices from product/meta
        raw_regular_price = product.get('regular_price') or product.get('price') or meta_lookup.get('_bp_regular_price') or meta_lookup.get('_bp_price')
        raw_sale_price = product.get('sale_price') or meta_lookup.get('_bp_sale_price')

        # Optional pricing rule from Step 2 source block
        source_info = product.get('source') or {}
        purchase_price = source_info.get('purchase_price')
        rrp_price = source_info.get('rrp_price')
        ...
        try:
            ...
            if calc_price is not None and rrp is not None:
                effective_regular_price = max(calc_price, rrp)
            elif calc_price is not None:
                effective_regular_price = calc_price
            elif rrp is not None:
                effective_regular_price = rrp

            if effective_regular_price is not None:
                rounded = self._round_up_to_99(effective_regular_price)
                if rounded is not None:
                    effective_regular_price = rounded
        except Exception:
            pass

        rp = self._as_price_str(effective_regular_price)
        sp = self._as_price_str(raw_sale_price)
        if rp:
        ...
        attr_name_map = {
            "size": "Suurus",
            "gender": "Sugu",
            "assamblee_on_vajalik": "Vajalik montaaž",
            "assembly_required": "Vajalik montaaž",
            "parcel_or_pallet": "Pakkimise tüüp",
            "number_of_packages": "Pakkide arv",
            "product_volume": "Toote maht",
        }
        for a in attrs:
            try:
                nm = self._norm(a.get('name'))
                nm_norm = nm.lower().replace(" ", "_")
                nm = attr_name_map.get(nm_norm, nm)
                if not nm:
                    continue
                if nm.lower() in EXCLUDED_ATTR_NAMES:
                    continue
                options = None
                # Prefer explicit options list
                if isinstance(a.get('options'), list):
                    options = [self._norm(x) for x in a.get('options') if self._norm(x)]
                # Fall back to new standard values list
                if not options and isinstance(a.get('values'), list):
                    options = [self._norm(x) for x in a.get('values') if self._norm(x)]
                # Final fallback to single value field
                ...
                if nm_norm == "size":
                    cleaned = []
                    for opt in options:
                        key = self._norm(opt).lower()
                        if key in {"n/a", "na", "-", ""}:
                            continue
                        cleaned.append(opt)
                    options = cleaned
                if nm_norm == "parcel_or_pallet":
                    translated: List[str] = []
                    for opt in options:
                        key = self._norm(opt).lower()
                        if key == "parcel":
                            translated.append("pakk")
                        elif key == "pallet":
                            translated.append("alus")
                        else:
                ...
                if nm_norm == "gender":
                    ...
                    for opt in options:
                        key = self._norm(opt).lower()
                        if key in {"unisex", "uni-sex", "uni sex"}:
                            translated.append("Unisex")
                        elif key in {"male", "man", "men", "boy"}:
                            translated.append("Mees")
                        elif key in {"female", "woman", "women", "girl"}:
                            translated.append("Naine")
                        else:
                            translated.append(opt)
                    options = translated
                if not options:
                    continue
                if nm.lower() in {"materjal", "material"}:
                    split_opts: List[str] = []
                    for opt in options:
                        if not opt:
                            continue
                    ...
                    options = deduped
                if nm.lower() in {"värvus", "värv", "color", "colour"}:
                    split_opts: List[str] = []
                    for opt in options:
                        if not opt:
                            continue
        ...
        # Categories: use translated path (fallback to original path) and create missing nodes on demand
        cat_obj = product.get('category') or {}
        raw_path = self._norm(cat_obj.get('path'))
        ...
        if (not product_exists) or update_images:
            ...
            try:
                if isinstance(raw_imgs, list) and len(raw_imgs) > 1:
                    name_lc = (name or "").lower()
                    if "päikesepuri" in name_lc:
                        raw_imgs = raw_imgs[:]
                        raw_imgs[0], raw_imgs[1] = raw_imgs[1], raw_imgs[0]
            except Exception:
                pass
