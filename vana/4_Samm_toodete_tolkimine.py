...
OPENAI_SERVICE_TIER = "flex"  # Kasuta "flex", "standard", "priority" või "auto"
...
def clean_product_description(html):
    """
    Eemaldab HTML-st inline CSS atribuudid, parandab piltide src ja
    tõlgib tüüpilised ingliskeelsed sildid/pealkirjad eesti keelde, et
    vältida ingliskeelsete jääkide kuvamist.
    """
    if not html:
        return html
    ...
    # Paranda <img src> väärtused, kui algavad suhtelise teega
    def fix_img_src(match):
        src = match.group(1)
        if not src.startswith("http"):
            src = "https://b2b.innpro.eu" + src
        return f'src="{src}"'
    cleaned_html = re.sub(r'src="([^"]+)"', fix_img_src, cleaned_html)
    ...
    # Heuristiline ingl->esti jääkide asendus (case-insensitive, täissõna)
    replacements = {
        r"\bName\b": "Nimi",
        r"\bModel\b": "Mudel",
        r"\bUsage\b": "Kasutamine",
        r"\bTechnical\s+data\b": "Tehnilised andmed",
        r"\bTechnical\s+specifications\b": "Tehnilised spetsifikatsioonid",
        r"\bDimensions\b": "Mõõdud",
        r"\bMaterial\b": "Materjal",
        r"\bWeight\b": "Kaal",
        r"\bColor\b": "Värvus",
        r"\bVolume\b": "Maht",
        r"\bCompatibility\b": "Ühilduvus",
        r"\bWarranty\b": "Garantii",
        r"\bEAN\b": "EAN",
        r"\bPackage\s+contents\b": "Komplektis",
        r"\bIncluded\s+in\s+the\s+box\b": "Komplektis",
        r"\bNumber\s+of\s+bags\s+per\s+roll\b": "Kotid rullis",
        r"\bNumber\s+of\s+rolls\s+in\s+the\s+set\b": "Rulle komplektis",
        r"\bBags\b": "Kotid",
        r"\bRolls\b": "Rullid",
    }
    for pat, et in replacements.items():
        cleaned_html = re.sub(pat, et, cleaned_html, flags=re.IGNORECASE)
...
def normalize_image_src(raw: str) -> str:
    s = (raw or "").strip()
    if not s:
        return ""
    if s.startswith("//"):
        return "https:" + s
    if s.startswith("/"):
        return "https://b2b.innpro.eu" + s
    if s.startswith("http"):
        lowered = s.lower()
        if "b2b.innpro.eu" in lowered or "b2b.innpro.eudata" in lowered:
...
def normalize_description_images(desc_html: str, fallback_alt: str, fallback_title: str) -> str:
    if not desc_html:
        return desc_html

    fallback_candidates = [
        (fallback_alt or "").strip(),
        (fallback_title or "").strip(),
    ]
    fallback_text = next((c for c in fallback_candidates if c), "")
    if fallback_text:
        fallback_text = _truncate_soft(fallback_text, 120)

    def _replace_img(tag_match: re.Match) -> str:
        tag = tag_match.group(0)

        def _replace_src(src_match: re.Match) -> str:
            quote = src_match.group(1)
            src_val = src_match.group(2)
            normalized = normalize_image_src(src_val)
            return f'src={quote}{html.escape(normalized, quote=True)}{quote}'

        if re.search(r'\bsrc=[\'\"]([^\'\"]+)[\'\"]', tag):
            tag = re.sub(r'\bsrc=([\'\"])([^\'\"]+)[\'\"]', _replace_src, tag, count=1)

        alt_match = re.search(r'\balt=([\'\"])([^\'\"]*)[\'\"]', tag)
        if alt_match:
            alt_text = alt_match.group(2).strip()
            if not alt_text and fallback_text:
                quote = alt_match.group(1)
                escaped = html.escape(fallback_text, quote=True)
                tag = re.sub(r'\balt=([\'\"])([^\'\"]*)[\'\"]', f'alt={quote}{escaped}{quote}', tag, count=1)
        elif fallback_text:
            escaped = html.escape(fallback_text, quote=True)
            if tag.endswith('/>'):
                tag = tag[:-2] + f' alt="{escaped}" />'
            else:
                tag = tag[:-1] + f' alt="{escaped}">' 
        return tag

    return re.sub(r'<img\b[^>]*?>', _replace_img, desc_html)
...
def process_one_product(prod: Dict[str, Any], index: int) -> Dict[str, int]:
    ...
    if run_prefixes:
        src_cat = str(((prod.get("source") or {}).get("prenta_category_path")) or "")
        normalized_src = normalize_prefix(src_cat)
        if not any(normalized_src.startswith(pref) for pref in run_prefixes):
            return {"added": 0, "skipped_existing": 0}
    ...
    product_description_flix = str(prod.get("description_flix") or "").strip()
    desc_len = _html_text_length(product_description)
    flix_len = _html_text_length(product_description_flix)
    ...
    try:
        ...
        brand_name = ""
    # Vastendused kasutaja terminitele
    producer = brand_name
    ...
    use_web_search = USE_STEP1_WEB_SEARCH or (desc_len < 160 and flix_len < 160)
    ...
    try:
        if use_web_search:
            instructions_text = (
                ...
                Kogutud materjali põhjal tagasta (kõik väärtused eesti keeles): 
                - 5 otsingufraasi (1 peamine, 4 lisa), mis oleksid kõige tõenäolisemad fraasid, mida kasutaksid nii AI kui ka kliendid toote leidmiseks (kasuta long-tail lähenemist ja lisa probleemipõhiseid ning kasupõhiseid päringuid). Väljendid peavad olema loomulikus eestikeelses vormis ja võivad sisaldada vajalikke mudeli- ning tehnilisi tähiseid, kuid väldi ingliskeelseid täislauseid.
                - 3 kõige tõenäolisemat ja levinumat küsimust ja vastust eesti keeles Q&A sektsiooni, mida kasutajad võiksid küsida. Vähemalt 1 küsimus esita selliselt, mis sellele vastates saaks seletada, millist probleemi toode lahendab või milline on toote suurim kasutegur.
            ...
            instructions_text = (
                ...
                Töötle olemasolevat toote alginfot ja koonda sellest kontekst, mis aitab hilisemates sammudes luua AI-ajastule vastava müügikirjelduse.
                ...
                - 5 otsingufraasi (1 peamine, 4 lisa), mis oleksid toote jaoks loogilised ja realistlikud, tuginedes ainult olemasolevale infole. Väljendid peavad olema loomulikus eestikeelses vormis ja võivad sisaldada vajalikke mudeli- ning tehnilisi tähiseid.
                - 3 kõige tõenäolisemat ja levinumat küsimust ja vastust eesti keeles Q&A sektsiooni.
    ...
    title_response = create_with_retry(
        ...
        f"""
            ...
            - Lühikirjeldus peaks andma kliendile kiire ja täpse ülevaate tootest. Kus, kellele ja miks toodet kasutatakse. Mis muret see lahendab ja mis on kliendi peamine kasu sellest tootest.
            - Väldi klišeesid nagu "nagu pildil näha", "pildilt on nähtav" jne   
            - Ara kasuta kirjelduses semikoolonit ";"! Lõpeta mõte punktiga ja alusta uue lausega.           
            ...
            Väljund: Tagasta JSON, kus "translated_title" sisaldab tootenimetust ja "short_description" sisaldab toote lühikirjeldust.
        """,
        input="Genereeri tõlgitud andmete põhjal tootenimi ja toote lühikirjelduse.",
        text={
            "verbosity": "low",
            "format": {
                "type": "json_schema",
                ...
                "schema": {
                    ...
                    "required": ["translated_title", "short_description"],
                    "additionalProperties": False
                },
                "strict": True
            }
    ...
    description_response = create_with_retry(
            ...
            Rakenda järgmisi tootekirjelduse parimaid praktikaid:
                ...
                Struktuur ja kohustuslikkuse reeglid:
                    - Kohustuslikud plokid:
                        1. Ava plokk: <h3> pealkiri, mis seob toote kasuteguri lahendatava probleemiga (sisalda main queryd) + järgnevalt <p>, mis kirjeldab väärtuspakkumist.
                    ...
                    - Tingimuslikud plokid (kasuta ainult siis, kui sisendmaterjal seda võimaldab):
                        • Originaalkirjelduse tõlge: kui algses kirjelduses on pilte või lõike, esita need järjekorras <img> → <h3> → <p> säilitades algse info; iga pildi alt-tekst tõlgi ja kohanda konteksti.
                        • Q&A teemade laiendus: loon h3 + 2–3 lõiku, mis vastavad STEP 1 Q&A põhipunktidele (ühilduvus, lisavarustus, hooldus jms).
                        • Paigaldus ja kasutus: h3 + lõik või loetelu praktiliste sammudega.
                        • Komplektis sisalduv: h3 + loetelu või lõik, mis kirjeldab komplekti.
                        • CTA plokk: h3 + lõik, mis võtab peamised kasutegurid kokku ja suunab ostule.
        ...
        """,
        input=f"Koosta kogu meie kogutud andmete põhjal eestikeelne tootekirjeldus.",
        text={
            "verbosity": "medium",
            "format": {
                "type": "json_schema",
    ...
    description_with_alt = normalize_description_images(
    ...
    # STEP 4: Koosta SEO title ja meta kirjeldus
    # --------------------------------------------------------------
    log(f"STEP 4: SEO meta kirjeldus (SKU {sku})")
    seo_meta_response = create_with_retry(
        _step_key="step4_seo_meta", _sku=sku,
        model="gpt-5",
        reasoning={"effort": "medium"},
        ...
        instructions=
        """
            Sinu ülesandeks on koostada SEO jaoks optimeeritud "SEO Title" ja "SEO Meta kirjeldus", mida kuvatakse otsingutulemustes.

            ✅ SEO Title nõuded:
            - SEO Title võib olla maksimaalselt 60 tähemärki pikk (ideaalis 50–60 tähemärki). Palun loe tähemärgid üle, sest see on range nõue, et tagada selle korrektne kuvamine otsingutulemustes.
            - SEO Title peab sisaldama kindlasti loodud SEO search query loomulikul viisil.
            - SEO Title peab olema atraktiivne, selge ja täpselt toote sisu kirjeldav.
            - Väldi ebaloomulikku keelt, ära tee sõna-sõnalist tõlget ja kohanda see idiomaatiliselt õigesse eesti keelde.
            - Vajadusel lühenda või lihtsusta loodud tootenime, et see vastaks SEO Title pikkusele.

            Koosta lühike, atraktiivne ja informatiivne SEO meta-kirjeldus.

            ✅ **Meta-kirjelduse nõuded:**
            - Peab olema maksimaalselt 160 tähemärki pikk. Palun loe tähemärgid üle ja veendu, et see ei ületaks seda piiri.
            - Sisaldab kindlasti loodud SEO search query loomulikul ja grammatiliselt õigel viisil.
            - Tekst peab olema kutsuv, selge ja mitte liiga reklaamilik.
            - Meta-kirjeldus peab andma täpse ülevaate tootest, et innustada klienti lingile klõpsama.
            - Väldi semikoolonit. Olulistes märksõnades kasuta boldi (max 3 korda). 
        """,
        input="Genereeri SEO Title ja SEO meta kirjeldus",
        text={
            "verbosity": "low",
            "format": {
                "type": "json_schema",
                "name": "seo_meta_schema",
                "schema": {
                    "type": "object",
                    "properties": {
    ...
    add_usage(seo_meta_response)
    record_usage("STEP 4: seo meta", seo_meta_response)

    try:
        meta_data = json.loads(seo_meta_response.output_text)
        seo_title = clean_double_asterisks(meta_data.get("seo_title", "").strip())
        seo_meta = clean_double_asterisks(meta_data.get("seo_meta", "").strip())
    except (json.JSONDecodeError, KeyError):
    ...
    if USE_STEP5_FINAL_REVIEW:
        ...
        final_response = create_with_retry(
            ...
            model="gpt-5",
            reasoning={"effort": "medium"},
            previous_response_id=seo_meta_response.id,
            instructions=
            """
            ...
            if to_translate:
                ...
                try:
                    attr_translate_response = create_with_retry(
                        ...
                        model="gpt-5",
                        reasoning={"effort": "medium"},
                        previous_response_id=(final_response.id if final_response else seo_meta_response.id),
                        instructions="""
    ...
    # --------------------------------------------------------------
    # STEP 8: Struktureeri ja täienda atribuudid kontekstist
    # --------------------------------------------------------------
    ...
    if qa:
        meta = ensure_meta(meta, "_bp_qa", json.dumps(qa, ensure_ascii=False))
    prod["meta_data"] = meta
