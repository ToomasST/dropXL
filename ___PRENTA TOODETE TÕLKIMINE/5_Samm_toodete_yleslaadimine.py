...
class WooCommerceUploader:
    ...
    def create_product_payload(self, product: dict, product_exists: bool = False, status: str = "publish", update_images: bool = False, sku_override: str = None):
        ...
        # Price mapping and rule: compare RRP vs (purchase_price * (1+markup) * (1+VAT))
        meta_lookup = {}
        for m in in_meta:
        ...
        try:
            if purchase_price is not None and rrp_price is not None:
                try:
                    pp = float(str(purchase_price))
                    rrp = float(str(rrp_price))
                    calc_price = pp * (1.0 + PRICE_MARKUP_RATE) * (1.0 + PRICE_VAT_RATE)
                    # If purchase+markup+VAT is greater than RRP, use that, otherwise use RRP
                    effective_regular_price = calc_price if calc_price > rrp else rrp
                except Exception:
                    pass
        except Exception:
            pass
